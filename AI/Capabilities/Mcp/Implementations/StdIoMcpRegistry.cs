...
/// <summary>
/// Registry de MCPs via transport StdIO (processo local lançado com command + args).
/// O endpoint é codificado como <c>cmd|arg1|arg2|...</c> dentro de <see cref="McpDescriptor.Endpoint"/>.
/// Exemplo: <c>npx|-y|@modelcontextprotocol/server-github</c>.
/// </summary>
public sealed class StdIoMcpRegistry(ILogger<StdIoMcpRegistry> logger) : McpRegistryBase
{
    ...
    {
        if (descriptor.Transport != McpTransport.StdIo)
            throw new ArgumentException($"StdIoMcpRegistry only accepts StdIo descriptors (got {descriptor.Transport})");
        ...
        var parts = descriptor.Endpoint.Value.Split('|', StringSplitOptions.RemoveEmptyEntries);
        if (parts.Length == 0)
            throw new ArgumentException($"StdIo endpoint must encode command and args as 'cmd|arg1|arg2' (got '{descriptor.Endpoint.Value}')");
        ...
        logger.LogInformation("Registered StdIo MCP {Name} with {Count} tools.", descriptor.Name, tools.Count);
    }
