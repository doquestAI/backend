...
namespace DoQuest.Tests.Application;
...
public sealed class SendMessageCommandHandlerTests
{
    ...
    public async Task Handle_WhenUserAndPlanValid_ShouldReturnSuccessWithReply()
    {
        ...
        _docRepo.SearchSimilarChunksAsync(Arg.Any<float[]>(), Arg.Any<int>(), Arg.Any<Guid?>(), Arg.Any<CancellationToken>())
            .Returns(new List<DoQuest.Domain.Entities.DocumentChunk>().AsReadOnly() as IReadOnlyList<DoQuest.Domain.Entities.DocumentChunk>);
